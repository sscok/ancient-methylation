...
genes=c()
#write.table(unique(input$V6),"genes",col.names=F, row.names=F, quote=F)
for (c_gene in unique(input$V6)){ 											# per gene
 ...
 captureOutput(temp, file = args[2], append=TRUE)
 genes=append(genes,c_gene)
}
write.table(genes,args[3],col.names=F, row.names=F, quote=F)
